...
RUN dotnet build "./FoodSelection.csproj" -c release -o /second_microserv/build
...
WORKDIR "/src/FoodSelectionAPI"
RUN dotnet publish "./FoodSelection.csproj" -c release -o /second_microserv/publish


FROM mcr.microsoft.com/dotnet/aspnet:8.0
WORKDIR /second_microserv
COPY --from=publish /second_microserv/publish .
EXPOSE 8080
